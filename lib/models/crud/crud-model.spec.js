...
describe('Crud Model', () => {
  ...
  it('calls "drop" on remove', async () => {
    ...
    jest.spyOn(TestCollection.collection, 'drop').mockResolvedValueOnce(MOCK_RETURN);
    try {
  ...
  describe('updateOne', () => {
    ...
    it('acknowledged === true', async () => {
      testCreatedObject = await TestCollection.createOne(testObj);
      testUpdateRes = await TestCollection.updateOne({ _id: testCreatedObject.insertedId }, updateObj);
