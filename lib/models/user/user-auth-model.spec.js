...
describe('User Model', () => {
  ...
  beforeAll(async () => {
    ...
    TestMongoClient = await setupTestDB({ ...dbObj });
    TestNlpDb = TestMongoClient.registerDB(DB_NAME);
    Cat = new User({ db: TestNlpDb, collection: COLL_NAME });
  ...
  describe('methods', () => {
    ...
    it('createOne: fails with invalid email string', async () => {
      try {
        ...
        expect(e.message).toBe(`Cannot call User createOne without a valid email address`);
      }
    });

    it('createOne: fails with duplicate key', async () => {
      try {
        await Cat.createOne({ email: 'failingEmail@email.com' });
        await Cat.createOne({ email: 'failingEmail@email.com' });
      } catch (e) {
        expect(e.message).toBe(`Cannot Create User`);
      }
    });
    ...
    describe('setPW', () => {
      ...
      describe('fails without', () => {
        it('email param', async () => {
          try {
            await Cat.setPW({ password: 'newPW' });
          } catch (e) {
            expect(e.message).toBe('cannot call User.setPW without email or pw');
          }
        });
    ...
    describe('validatePW', () => {
      ...
      describe('fails without', () => {
        it('email param', async () => {
          try {
            await Cat.validatePW({ password: 'newPW' });
          } catch (e) {
            expect(e.message).toBe('cannot call User.validatePW without email or pw');
          }
        });
    ...
    it('requestPwReset', () => {
      ...
      expect(res).toBe('User requestPwReset Here');
    });

    it('saltPw', () => {
      let { salt, saltedPw } = Cat.saltPw('testPw');
      expect(typeof salt).toBe('string');
      expect(typeof saltedPw).toBe('string');
    });
  });
