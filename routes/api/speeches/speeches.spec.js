...
describe("speeches", () => {
  ...
  it("POST fail: returns a 500 when module throws", async () => {
    jest.unstable_mockModule("./../../../state.js", () => ({
      get: () => ({
        insertOne: mockSpeechesFn.mockRejectedValueOnce({
          message: "mock thrown",
        }),
      }),
    }));
    const { statusCode, body, ...rest } = await supertest(app)
      .post(SPEECHES_URL)
      .send({
        text: "this is a test",
        author: "test author",
  ...
  it('POST success: returns a 200 with location header, calls "runAnalytics fn"', async () => {
    ...
    jest.unstable_mockModule("./../../../state.js", async () => ({
      get: () => ({
        insertOne: mockSpeechesFn.mockImplementation(() => ({
          insertedId: mockPostSpeechId,
        })),
      }),
      stateObj: {},
    }));
    ...
    expect(statusCode).toBe(200);
    expect(headers.location.includes(`/speeches/${mockPostSpeechId}`)).toBe(
      true,
    );
