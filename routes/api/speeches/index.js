...
async function getSpeeches(req, res) {
  let data = await get('Speeches').find({}).project({ _id: 1, author: 1, date: 1 }).toArray();
  return res.status(200).json(data);
}

const speechesRouter = Router();

function getSpeechesAnalytics(req, res) {
  res.status(200).json('speeches analytics here...');
  return;
}
speechesRouter.get('/analytics', getSpeechesAnalytics);
